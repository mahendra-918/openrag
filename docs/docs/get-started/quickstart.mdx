...
3. Once the environment is ready, and the OpenRAG terminal session starts, select the **Configure** option.
...
6. At the `Start services now?` prompt, press <kbd>Y</kbd> to start the OpenRAG services.
...
7. Select the **Open OpenRAG in browser** option to launch the OpenRAG application and start the application onboarding process.
...
9. Click through the onboarding chat for a brief introduction to OpenRAG, or click <Icon name="ArrowRight" aria-hidden="true"/> **Skip overview**.
...
3. To verify the agent's response, click <Icon name="Library" aria-hidden="true"/> **Knowledge** to view the documents stored in the [OpenRAG OpenSearch knowledge base](/knowledge).
